...
def constructRepVsRest():
    ...
    for repId in reportIds:
        ...
        rep_url_templates = rep_id_vs_templates[repId]
        no_of_templates = len(rep_url_templates)
        for i in range(no_of_templates):
            t = rep_url_templates[i]
            id_c += 1
            content_c += '<c '
            content_c += 'a="{0}" b="{1}" c="{2}" '.format(id_c, repId, t)
            if i == no_of_templates-1 and repId in report_vs_transformer_model:
                content_c += "TRANSFORMER_MODEL='{0}' ".format(report_vs_transformer_model[repId])
            content_c += '/>\n'
    print('id_c : ', id_c)
def start():
    global report_processed
    populate_tuples()
    removeDuplicates()
    report_processed={}
    constructRepVsRest()
